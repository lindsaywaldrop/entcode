...
# The hair where sampling ended (e.g. hair 3 for row 1) should be "1".
...
for (j in 1:n){		# Main loop
  ...
  for (arrayrow in 1:norows){
    ...
    for (k in 1:hairsinrow){  
      ...
      new.curve<-predict(curve,seq(min(data1$V1[stpt:endpt]),max(data1$V1[stpt:endpt]),by=0.00005)) # Resamples to create smooth spline curve
...
# Save data!
write.table(shear_hair2,file=paste("shearhairs-",n,"-",Sys.Date(),".csv",sep=""),sep=",")
# Quits R
#quit(save="no")
#################################################################################################################

...
Plot old and new versions of calculations together
...
load_results <- function(hairno, run_id){
  ...
  hair_dots <- sum(hair_conc$hairdots)
  cmax <- hair_conc$cmax
  ctotal <- hair_conc$ctotal 
  domain_area <- hair_conc$xlength * hair_conc$ylength
  total_conc_old <- hair_values * as.vector(hair_dxdy * hair_dots)
  total_conc_new <- (hair_values * as.vector(hair_dxdy * c_init_pts)) / as.vector(ctotal)
  norm_conc_old <- total_conc_old / as.vector(ctotal * domain_area)
  norm_conc_new <- total_conc_new / as.vector(cmax)
  ...
  hair_totals <- data.frame("time" = print_times, 
                        ...
                        "total_conc_old" = total_conc_old,
                       "norm_conc_old" = norm_conc_old,
                       "total_conc_new" = total_conc_new,
                       "norm_conc_new" = norm_conc_new)
...
assemble_conv_plots <- function(sim_dat){
  ...
  p_total_old <- ggplot(sim_dat, aes(time, total_conc_old, color = resolution)) + geom_point() +
  scale_color_viridis(discrete = T, drop = F, name = "Grid\nresolution") +
  #annotate("text", x=0.1, y=5.5e-4, label = "D = 4.99e-5\ngap = 10.4") +
  ggtitle(paste("Simulation", sim_dat$simulation[1])) +
  theme_bw()
  
  p_norm_old <- ggplot(sim_dat, aes(time, norm_conc_old, color = resolution)) + geom_point() +
  scale_color_viridis(discrete = T, drop = F, name = "Grid\nresolution") +
  #annotate("text", x=0.1, y=5.5e-4, label = "D = 4.99e-5\ngap = 10.4") +
  ggtitle(paste("Simulation", sim_dat$simulation[1])) +
  theme_bw()
  
  p_total_new <- ggplot(sim_dat, aes(time, total_conc_new, color = resolution)) + geom_point() +
  scale_color_viridis(discrete = T, drop = F, name = "Grid\nresolution") +
  #annotate("text", x=0.1, y=5.5e-4, label = "D = 4.99e-5\ngap = 10.4") +
  ggtitle(paste("Simulation", sim_dat$simulation[1])) +
  theme_bw()
  
  p_norm_new <- ggplot(sim_dat, aes(time, norm_conc_new, color = resolution)) + geom_point() +
  scale_color_viridis(discrete = T, drop = F, name = "Grid\nresolution") +
  #annotate("text", x=0.1, y=5.5e-4, label = "D = 4.99e-5\ngap = 10.4") +
  ggtitle(paste("Simulation", sim_dat$simulation[1])) +
  theme_bw()
  return((p_raw + p_total_old + p_norm_old)/(plot_spacer() + p_total_new + p_norm_new) + plot_layout(guides="collect"))
}

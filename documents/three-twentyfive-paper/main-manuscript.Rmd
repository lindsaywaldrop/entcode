...
sobol_labels <- data.frame(
  "short" = c("angle", "gap", "ant", "dist", "re", "diff_coef", "stink_width", 
              "init_conc"), 
  "long" = c("Angle to flow", "Gap ratio", "Antenna ratio", "Distance ratio", 
                  "Reynolds number", "Diffusion coefficient", "Odor-stripe width", 
...
plot_parameter <- function(three_dat, twofive_dat, parameter, output, 
                           sobol_labels, equal_yaxis = NULL, col_opt=NULL){
  parameter_label <- sobol_labels$long[sobol_labels$short == parameter]
  output_label <- ifelse(output == "norm_conc", "Normalized\nconcentration", 
                         "Total\nconcentration")
  if(is.null(col_opt)){
    p3 <- ggplot(three_dat, aes(.data[[parameter]], .data[[output]])) + 
      geom_point() +
      xlab(parameter_label) + ylab(output_label) +
    ggtitle("3 hair") + theme_minimal()
    p25 <- ggplot(twofive_dat, aes(.data[[parameter]], .data[[output]])) + 
      geom_point() + 
      ylab(" ") + xlab(parameter_label) +
...
```{r norm-conc-SI, echo = FALSE, fig.height=9, fig.width=6, fig.cap="Normalized concentration and sobol indices"}
p1 <- plot_parameter(three_hair, twofive_hair, "angle", "norm_conc", sobol_labels, equal_yaxis = F) 
p2 <- plot_parameter(three_hair, twofive_hair, "re", "norm_conc", sobol_labels, equal_yaxis = F) 
p3 <- plot_parameter(three_hair, twofive_hair, "gap", "norm_conc", sobol_labels, equal_yaxis = F)
p4 <- ggplot(SI_data[SI_data$output == "norm_conc",], aes(x = parameter, y = value, 
                                                          fill = array)) +
  geom_bar(position = "dodge", stat= "identity") +
  scale_x_discrete(labels = sobol_labels$long) +

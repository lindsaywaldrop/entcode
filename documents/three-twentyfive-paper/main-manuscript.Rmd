...
#three_hair$total_conc <- three_hair$raw_total_conc * (three_hair$hair_dxdy * 
#                                                        three_hair$hair_dots)
#twofive_hair$total_conc <- twofive_hair$raw_total_conc * (twofive_hair$hair_dxdy #* twofive_hair$hair_dots)
#three_hair$norm_conc <- three_hair$total_conc / (three_hair$ctotal * three_hair$domain_area)
#twofive_hair$norm_conc <- twofive_hair$total_conc / (twofive_hair$ctotal * 
#                                                       twofive_hair$domain_area)

twofive_hair$total_conc_ratio <- twofive_hair$total_conc/three_hair$total_conc
twofive_hair$total_conc_ratio[which(is.infinite(twofive_hair$total_conc_ratio))] <- NA
twofive_hair$norm_conc_ratio <- twofive_hair$norm_conc/three_hair$norm_conc
...
```{r total-conc-angle, echo = FALSE, fig.cap="Total concentration and gap width"}
  plot_parameter(three_hair, twofive_hair, "angle", "total_conc", equal_yaxis = TRUE,
                 col_opt = "gap") /
  plot_parameter(three_hair, twofive_hair, "angle", "norm_conc", equal_yaxis = TRUE,
                 col_opt = "gap") + 
...
ptotal.ratio <- ggplot(twofive_hair[order(twofive_hair$total_conc_ratio),], 
                       aes(x = .data[["gap"]], y = .data[["angle"]], 
                                     color = .data[["total_conc_ratio"]], 
                                     size = .data[["total_conc_ratio"]])) + 
  geom_point()+
  scale_color_binned_divergingx(name = "Ratio", 
                                palette = "PuOr",
                                l1 = 30, l3 = 30,
                              mid = 1.0, breaks = c(0.5, 1, 2, 3),
                              n_interp = 4)+
  #scale_color_viridis_b("Normalized\nconcentration", breaks = seq(0, 0.0125, length.out = 5), 
  #                      limits = c(0,0.0125))+
  scale_size_binned("Ratio", breaks = c(0.5, 1, 2, 3), 
                    range = c(0.5, 4), limits = c(0.1, 6)) + 
  ggtitle("Total concentration 25/3 ratio") + theme_minimal() + 
  guides(color= guide_legend(), size=guide_legend()) 

pnorm.ratio <- ggplot(twofive_hair[order(twofive_hair$norm_conc_ratio),], aes(x = .data[["gap"]], 
                                                                          y = .data[["angle"]], 
  ...
  scale_size_binned("Ratio", breaks = c(0.5, 1, 2, 3), 
                    range = c(0.5, 4), limits = c(0.1, 6)) + 
  ggtitle("Normalized concentration 25/3 ratio") + theme_minimal() + 
  guides(color= guide_legend(), size=guide_legend()) 

ptotal.ratio + pnorm.ratio + plot_layout(guides = "collect")
ggsave("ratio-gap-angle-plots.pdf", height = 5, width = 9)
```
...
```{r norm-conc-gap-angle, echo = FALSE, fig.cap="Total concentration and gap width"}
plot_parameter(three_hair, twofive_hair, "gap", "norm_conc", equal_yaxis = TRUE) /
  plot_parameter(three_hair, twofive_hair, "angle", "norm_conc", equal_yaxis = T)
```
...
p3 <- ggplot(three_hair[order(three_hair$total_conc),], 
             aes(.data[["gap"]], .data[["angle"]], 
                 color = .data[["total_conc"]], size = .data[["total_conc"]])) + 
  geom_point() +
  scale_color_viridis_b("Total\nconcentration", breaks = seq(0, 4e7, length.out = 5), 
                        limits = c(0, 4e7)) +
  scale_size_binned("Total\nconcentration", breaks = seq(0, 4e7, length.out = 5), 
                    range = c(0.5, 4), limits = c(0, 4e7)) + 
  
  ggtitle("3-hair array") + theme_minimal() +
  guides(color= guide_legend(), size=guide_legend()) 


p25 <- ggplot(twofive_hair[order(twofive_hair$total_conc),], aes(.data[["gap"]], .data[["angle"]], 
              color = .data[["total_conc"]], size = .data[["total_conc"]])) + 
  geom_point() +
  scale_color_viridis_b("Total\nconcentration", breaks = seq(0, 2e8, length.out = 5), 
                        limits = c(0, 2e8)) +
  scale_size_binned("Total\nconcentration", breaks = seq(0, 2e8, length.out = 5), 
                    range = c(0.5, 4), limits = c(0, 2e8)) + 
  ggtitle("25-hair array") + theme_minimal() + 
  guides(color= guide_legend(), size=guide_legend()) 

(p3 / p25) + plot_layout(guides = "collect")
```


```{r gap-angle-norm-conc-map, echo = FALSE, fig.height=5*1.5, fig.width=5}
p3 <- ggplot(three_hair, 
             aes(.data[["gap"]], .data[["angle"]], 
                 color = .data[["norm_conc"]], size = .data[["norm_conc"]])) + 
  geom_point() +
  scale_color_viridis_b("Normalized\nconcentration", breaks = seq(0, 0.0025, length.out = 5), 
                        limits = c(0, 0.0025)) +
  scale_size_binned("Normalized\nconcentration", breaks = seq(0, 0.0025, length.out = 5), 
                    range = c(0.5, 4), limits = c(0, 0.0025)) + 
...
p25 <- ggplot(twofive_hair, aes(.data[["gap"]], .data[["angle"]], 
  ...
  scale_color_viridis_b("Normalized\nconcentration", breaks = seq(0, 0.0125, length.out = 5), 
                        limits = c(0,0.0125))+
  scale_size_binned("Normalized\nconcentration", breaks = seq(0, 0.0125, length.out = 5), 
                    range = c(0.5, 4), limits = c(0,0.0125)) + 
  ggtitle("25-hair array") + theme_minimal() + 
  guides(color= guide_legend(), size=guide_legend()) 

(p3 + p25) + plot_layout(guides = "collect")
ggsave("gap-angle-plots.pdf", height = 5, width = 9)
```



```{r max-25-array, echo=FALSE, warning=FALSE, fig.cap="Parameter combinations for total concentration captured (A-B) and normalized concentration captured (C-D) for the 25-hair array."}
#Total
plot_individual_hairs(1300, three_hair, twofive_hair, hdia) /
  # Normalized
  plot_individual_hairs(876, three_hair, twofive_hair, hdia) +
  plot_annotation(tag_levels = "A")
```

```{r max-3-array, echo=FALSE, warning=FALSE, fig.cap="Parameter combinations for total concentration captured (A-B) and normalized concentration captured (C-D) for the 3-hair array."}
#Total: 
plot_individual_hairs(645, three_hair, twofive_hair, hdia) /
  #Normalized:
  plot_individual_hairs(1061, three_hair, twofive_hair, hdia) + 
  plot_annotation(tag_levels = "A")
```

```{r total-norm-conc-gap, echo = FALSE, fig.cap="Total concentration and gap width"}
plot_parameter(three_hair, twofive_hair, "diff_coef", "total_conc", equal_yaxis = T) /
  plot_parameter(three_hair, twofive_hair, "diff_coef", "norm_conc", equal_yaxis = T)
ggsave("total_norm_conc_plot.pdf", height = 5, width=7)
```

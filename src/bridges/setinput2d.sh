...
cut -f 3 "$WD"/data/parameters/allpara_165.txt > Re.txt

# Count number of lines in files
numlines=$(grep -c "^" "$WD"/data/parameters/allpara_165.txt)
...
awk -v var="$i" 'NR==254 {$0="   log_file_name = \42../results/runs/IB2d.log"'"var"'"\42                //"} 1' input2dRe3 > input2dRe4
awk -v var="$i" 'NR==260 {$0="   viz_dump_dirname = \42../results/runs/viz_IB2d"'"var"'"\42                //"} 1' input2dRe4 > input2dRe5
